...
export class RequestLog {
    ...
    start(path: string, method: string, headers: any, params: any, body: any) {
        const _log = LogModel
            .generate(
                this._logService.environment,
                this._logService.appName,
                this._logService.appVersion,
                this._reqId, LogLevel.INFO)
            .method(method)
            .requestPath(path)
    ...
    info(msg: string, data: any = {}, operation: string = '') {
        let _log = LogModel
            .generate(this._logService.environment, this._logService.appName, this._logService.appVersion, this._reqId, LogLevel.INFO)
            .message(msg)
            .data(data)
            .operation(operation);
        this._logService.commitLog(_log);
